...
//
//		sql += "u_m_id, ";
//		args.add(userMemberVo.getU_m_id());
//
//		sql += "u_m_pw, ";
//		args.add(passwordEncoder.encode(userMemberVo.getU_m_pw()));
//
//		sql += "u_m_name, ";
//		args.add(userMemberVo.getU_m_name());
//
//		sql += "u_m_gender, ";
//		args.add(userMemberVo.getU_m_gender());
//
//		sql += "u_m_mail, ";
//		args.add(userMemberVo.getU_m_mail());
//
//		sql += "u_m_phone, ";
//		args.add(userMemberVo.getU_m_phone());
//
//		sql += "u_m_reg_date, u_m_mod_date) ";
//		sql += "VALUES(?, ?, ?, ?, ?, ?, NOW(), NOW())";
//
//		int result = -1;
//
...
//	}
	...
	public int updateUserAccount(int u_m_no) {
		System.out.println("[UserMemberDao] updateUserAccount()");

		String sql = "UPDATE tbl_user_member SET " + "u_m_approval=1 " + "WHERE u_m_no=?";

		int result = -1;
		try {
			result = jdbcTemplate.update(sql, u_m_no);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return result;
